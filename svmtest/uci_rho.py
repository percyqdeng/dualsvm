...
dtname = ucifile[1]
dtpath = ucipath
data = scipy.io.loadmat(dtpath + dtname+'.mat')
x = data['x']
...
rholist = [0.01, 0.02, 0.05, 0.07, 0.1, 10, ]
n_iter = 10
...
# plt.show()
plt.savefig('svmtest/output/%s_obj_%2.2f_gm_%2.2f.eps' % (dtname, C, gamma))
...
plt.ylabel('dual objective')
# plt.show()
plt.savefig('svmtest/output/%s_obj2_C_%2.2f_gm_%2.2f.eps' % (dtname, C, gamma))
...
plt.xscale('log')
plt.yscale('log')
plt.savefig(os.path.join('svmtest', 'output', '%s_train_C_%2.2f_gm_%2.2f.eps'
                         % (dtname, C, gamma)))
...
plt.yscale('log')
plt.xlim(xmin=1)
plt.savefig('svmtest/output/%s_test_C_%2.2f_gm_%2.2f.eps' % (dtname, C, gamma))
