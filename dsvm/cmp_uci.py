...
for k, (train_index, test_index) in enumerate(rs):
    ...
    n = train_index.size
    start_t = time.time()
    clf = DualKSVM(lmda=C/n, kernel='rbf', gm=gamma, nsweep=n, b=4, verbose=True, rho=rho1, algo_type='scg_da')
    ...
    start_t = time.time()
    clf2 = DualKSVM(lmda=C/n, kernel='rbf', gm=gamma, nsweep=int(4*n), algo_type='cd')
...
plt.savefig('../output/ksvm_usps_conv_%d_%d_C_%2.2f_gm_%2.2f.eps' %(pos_class, neg_class, C, gamma))
...
plt.yscale('log')
plt.xlim(xmin=1)
plt.savefig('../output/ksvm_usps_error%d_%d_C_%2.4f_gm_%2.2f.eps' %(pos_class, neg_class, C, gamma))

# plt.errorbar(clf.nker_opers, testerr_dasvm[C].mean(), yerr=testerr_dasvm[C].std(axis=0), 'x-', label='scg')
# plt.semilogy(clf2.nker_opers, testerr_cd[C], 'o-', label='cd')
# plt.semilogy(clf3.nker_opers, testerr_pega[C], 'D-', label='Pegasos')
